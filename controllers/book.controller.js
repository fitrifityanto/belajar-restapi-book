...
const findBookById = async (req, res) => {
  try {
    ...
    if (!book) {
      res
        .status(404)
        .send({ message: `data buku id ${req.params.id} tidak ada` });
    } else res.send(book);
  } catch (error) {
    res.status(500).send({ message: "error menerima data" });
  }
...
const deleteBook = async (req, res) => {
  try {
    ...
    if (!book) {
      res.status(404).send({
        message: `data buku id ${req.params.id} gagal di hapus, mungkin data tidak ada`,
      });
    } else {
...
const deleteAll = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ message: error.message || "gagal menghapus semua data buku" });
  }
};
...
const updateBook = async (req, res) => {
  try {
    ...
    if (!req.body) {
      res
        .status(400)
        .send({ message: "data yang akan diubah tidak boleh kosoong" });
      return;
    }
    await conn;
    ...
    if (!book) {
      res.status(404).send({
        message: "gagal mengubah data buku, mungkin data buku tidak ada",
      });
